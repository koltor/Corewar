...
/*   Updated: 2019/10/14 12:56:09 by matheme     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void	aritmetical_substraction(t_process *process, t_data *arena_data, int vb)
{
	char ocp;
	char r[3];
	...
	if (r[0] <= 0 || r[1] <= 0 || r[2] <= 0 || r[0] >= 17 || r[1] >= 17 || r[2] >= 17)
		return ;
	process->reg[r[2] - 1] = process->reg[r[0] - 1] - process->reg[r[1] - 1];
	if (process->reg[r[2] - 1] == 0)
		process->carry = 1;
	else
		process->carry = 0;
	if (vb & VERBOSE_SHOW_OPERATIONS)
		FP("P%5d | sub r%hhd r%hhd r%hhd\n", process->id, r[0], r[1], r[2]);
}
